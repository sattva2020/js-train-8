...
function getUsersFromDatabase(array) {
  ...
  const userArray = Array.from(array, userRecords => ({
    id: userRecords.id,
    firstName: userRecords.name.toUpperCase(),
    years: userRecords.age
  }));
...
function filterByCondition(arr, condition) {
  ...
  let fiterArr = arr.filter(condition)
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (target < 0 || target >= arr.length || start < 0 || start >= arr.length || end <= start || end > arr.length) {
...
// function countAndLogOccurrences(arr, element) {
//   const count = arr.filter(item => item === element).length;
//   console.log(`Елемент "${element}" зустрічається ${count} раз(ів) в масиві.`);
// }

// const fruits = ["apple", "banana", "orange", "apple", "banana", "grape"];
// countAndLogOccurrences(fruits, "banana"); // Виведе: Елемент "banana" зустрічається 2 раз(ів) в масиві.
// countAndLogOccurrences(fruits, "apple");  // Виведе: Елемент "apple" зустрічається 2 раз(ів) в масиві.
// countAndLogOccurrences(fruits, "grape");  // Виведе: Елемент "grape" зустрічається 1 раз(ів) в масиві.
// countAndLogOccurrences(fruits, "pear");   // Виведе: Елемент "pear" зустрічається 0 раз(ів) в масиві.
